...
Ana::Ana(TTree *tree) : fChain(0)  {
   if (tree == 0) {
      TFile *f = (TFile*)gROOT->GetListOfFiles()->FindObject(Form("g4beamline.root"));
      if (!f || !f->IsOpen()) {
         f = new TFile(Form("g4beamline.root"));
      }
      TDirectory * dir = (TDirectory*)f->Get(Form("g4beamline.root:/VirtualDetector")); 
